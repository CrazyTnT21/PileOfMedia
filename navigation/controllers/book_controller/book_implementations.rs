...
//TODO: Refactor
pub fn get_mut_service<'a>(transaction: &'a Transaction<'a>, client: &'a Client, display_path: &'a str, path: &'a str) -> impl MutBookService + 'a {
  let image_repository = Arc::new(get_image_repository(client));
  ...
  let character_repository = Arc::new(get_character_repository(client, DEFAULT_LANGUAGE, image_repository.clone()));
  let mut_book_character_repository = Arc::new(get_mut_book_character_repository(transaction));
  let mut_image_repository = Arc::new(get_mut_image_repository(transaction, image_repository.clone(), mut_file_repository, file_repository));
  ...
  let franchise_repository = Arc::new(get_franchise_repository(client, DEFAULT_LANGUAGE));
  let book_repository = Arc::new(get_book_repository(client, DEFAULT_LANGUAGE, image_repository, franchise_repository.clone()));
  let theme_repository = Arc::new(get_theme_repository(client, DEFAULT_LANGUAGE));
  let mut_book_theme_repository = Arc::new(get_mut_book_theme_repository(transaction));
  let mut_book_repository = Arc::new(get_mut_book_repository(transaction, DEFAULT_LANGUAGE, mut_book_genre_repository, mut_book_character_repository, mut_book_theme_repository, mut_book_involved_repository, book_repository.clone()));
  get_mut_book_service(DEFAULT_LANGUAGE, book_repository, mut_book_repository, mut_image_service, franchise_repository, theme_repository, genre_repository, character_repository, person_repository, role_repository)
}
