import {Component, Input} from '@angular/core';
import {Base} from '../Base';
import {MainComponent} from "../../app/pages/main/main.component";
import {DisplayType} from "./table.component";

@Component({
  selector: 'app-taskbar',
  template: `
    <div class="col-12 lazy" style="flex-wrap: nowrap">
      <!--            <img src="data:image/svg+xml;base64,"-->
      <!--                 class="max"/>-->
      <div class="dropdown col" *ngFor="let page of Pages">
        <button class="fill"
                (click)="Tools.navigatePage(page.url)">
          <div *ngIf="page.name == null">{{page.url}}</div>
          <div *ngIf="page.name != null">{{page.name}}</div>
        </button>
        <div class="dropdown-content col-12">
          <button *ngFor="let child of page.children" class="fill">
            <div *ngIf="child.name == null">{{page.url}}</div>
            <div *ngIf="child.name != null">{{page.name}}</div>
          </button>
        </div>
      </div>
      <div class="col pad"><input/></div>

      <div class="dropdown">
        <button>Settings</button>
        <div class="dropdown-content">
          <button>Profile</button>
          <button>About</button>
        </div>
      </div>
      <div class="dropdown">
        <button>Language</button>
        <div class="dropdown-content">
          <button (click)="language('EN')">English</button>
          <button (click)="language('DE')">German</button>
          <button (click)="language('ES')">Spanish</button>
        </div>
      </div>
      <div class="dropdown">
        <button>View</button>
        <div class="dropdown-content">
          <button (click)="view(DisplayType.card)">Card</button>
          <button (click)="view(DisplayType.list)">List</button>
        </div>
      </div>
    </div>
    <app-login></app-login>
  `
})
export class TaskbarComponent extends Base {
DisplayType = DisplayType;
  @Input("Pages")
  Pages: Page[];

  constructor() {
    super();
  }

  language(value: string) {
    MainComponent.config.language = value;
    MainComponent.saveConfig();
  }

  view(value: DisplayType) {
    MainComponent.config.view = value;
    MainComponent.saveConfig();
  }
}

export interface Page {
  icon?: string;
  url: string;
  name?: string;
  children?: Page[];
  query?: any;

}


