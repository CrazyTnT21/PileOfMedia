...
pub mod create_person_part {
  ...
  use crate::entities::person::create_person::CreatePerson;
  ...
  impl multipart::FromMultiPart for CreatePerson {
    ...
    where
        Self: Sized,
    {
      let mut person: Option<CreatePerson> = None;
      let mut image = None;
      while let Some(a) = multipart.next_field().await.map_err(|x| CreatePersonPartError::OtherError(Box::new(x)))? {
        ...
        match part {
          CreatePersonPart::Person => {
            let create_person = serde_json::from_slice::<CreatePerson>(&a.bytes()
                .await
      ...
      let mut person = person.ok_or(CreatePersonPartError::PersonMissing)?;
      if let Some(data) = image {
        if let Some(ref mut person_image) = person.image {
          person_image.0 = data.to_vec();
        }
      };
      Ok(person)
    }
