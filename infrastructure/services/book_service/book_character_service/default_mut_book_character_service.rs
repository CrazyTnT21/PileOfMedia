...
impl<'a> DefaultMutBookCharacterService<'a> {
  pub fn new(
    ...
    mut_book_character_repository: Arc<dyn MutBookCharacterRepository + 'a>,
    character_repository: Arc<dyn CharacterRepository + 'a>,
  ) -> DefaultMutBookCharacterService<'a> {
    DefaultMutBookCharacterService {
      book_repository,
...
impl DefaultMutBookCharacterService<'_> {
  async fn validate_add(
    ...
    book_id: u32,
    characters: &[u32],
  ) -> Result<(), ServiceError<MutBookCharacterServiceError>> {
    self.validate(book_id, characters).await?;
    let associated = self
      .book_character_repository
      .filter_existing(book_id, characters)
      .await?;
  ...
  async fn validate_remove(
    ...
    if existing.len() != characters.len() {
      ...
      return Err(ServiceError::ClientError(MutBookCharacterServiceError::NotAssociated(
  ...
  async fn validate(&self, book_id: u32, characters: &[u32]) -> Result<(), ServiceError<MutBookCharacterServiceError>> {
    ...
    if ids.is_empty() {
      return Err(ServiceError::ClientError(
        MutBookCharacterServiceError::NonExistentBook(book_id),
      ));
    }
    if characters.is_empty() {
      return Err(ServiceError::ClientError(
        MutBookCharacterServiceError::NoCharactersProvided,
      ));
    }
    Ok(())
  }
