...
impl<'a> DefaultMutFranchiseRepository<'a> {
  pub fn new(transaction: &'a Transaction<'a>,
             default_language: Language,
             franchise_repository: Arc<dyn FranchiseRepository + 'a>, ) -> DefaultMutFranchiseRepository<'a> {
...
impl MutFranchiseRepository for DefaultMutFranchiseRepository<'_> {
  async fn create(&self, item: CreatePartialFranchise) -> Result<Franchise, Box<dyn Error>> {
    ...
    let franchise = self.franchise_repository
  ...
  async fn delete(&self, ids: &[u32]) -> Result<(), Box<dyn Error>> {
    ...
    Delete::new::<DbFranchiseTranslation>(Expression::new(ValueIn::new((DbFranchiseTranslation::TABLE_NAME, "fktranslation"), &ids)))
