...
impl<'a> DefaultMutThemeRepository<'a> {
  pub fn new(transaction: &'a Transaction<'a>,
             default_language: Language,
             theme_repository: Arc<dyn ThemeRepository + 'a>, ) -> DefaultMutThemeRepository<'a> {
...
impl MutThemeRepository for DefaultMutThemeRepository<'_> {
  async fn create(&self, item: CreatePartialTheme) -> Result<Theme, Box<dyn Error>> {
    ...
    let theme = self.theme_repository
  ...
  async fn delete(&self, ids: &[u32]) -> Result<(), Box<dyn Error>> {
    ...
    Delete::new::<DbThemeTranslation>(Expression::new(ValueIn::new((DbThemeTranslation::TABLE_NAME, "fktranslation"), &ids)))
