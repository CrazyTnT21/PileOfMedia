...
impl<'a> DefaultMutGenreRepository<'a> {
  pub fn new(transaction: &'a Transaction<'a>,
             default_language: Language,
             genre_repository: Arc<dyn GenreRepository + 'a>, ) -> DefaultMutGenreRepository<'a> {
...
impl MutGenreRepository for DefaultMutGenreRepository<'_> {
  async fn create(&self, item: CreatePartialGenre) -> Result<Genre, Box<dyn Error>> {
    ...
    let genre = self.genre_repository
  ...
  async fn delete(&self, ids: &[u32]) -> Result<(), Box<dyn Error>> {
    ...
    Delete::new::<DbGenreTranslation>(Expression::new(ValueIn::new((DbGenreTranslation::TABLE_NAME, "fktranslation"), &ids)))
