...
impl Display for MutFranchiseServiceError {
  fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
    write!(f, "{}", match self {
      ...
      MutFranchiseServiceError::NoTranslationInLanguageProvided(language) => format!("No translation in '{}' ({}) provided", language, language.language_code()),
      MutFranchiseServiceError::InvalidName(x) => format!("Name '{x}' in translation is invalid"),
      MutFranchiseServiceError::NonExistent(x) => format!("The following franchises do not exist: [{}]", x.join_comma()),
