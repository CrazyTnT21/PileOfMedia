...
impl Display for MutBookServiceError {
  fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
    write!(f, "{}", match self {
      ...
      MutBookServiceError::NonExistentCharacters(x) => format!("The following characters do not exist: [{}]", x.join_comma()),
      ...
      MutBookServiceError::NoTranslationInLanguageProvided(language) => format!("No translation in '{}' ({}) provided", language, language.language_code()),
      ...
      MutBookServiceError::NonExistentTranslationCover(language) => format!("A cover for the language '{language}' ({}) does not exist", language.language_code()),
